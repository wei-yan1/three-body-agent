...
这是一个以前后端一体化方式实现的小说人物 Agent 项目，目标不是做普通的剧情问答，而是让用户能在不同时间线阶段中和《三体》角色对话。项目目前支持罗辑、章北海等角色，用户可以在前端选择角色、时间阶段和知识模式，与“处在该阶段认知边界内的人物”进行交流。
...
设计并实现了一个基于 FastAPI、LangChain Agent Middleware、Chroma 向量数据库和 DashScope 系列模型的《三体》时序人格对话 Agent 系统。项目围绕“小说人物原汁原味复现”这一目标，将角色人格拆分为时间线阶段画像，并结合小说原文 RAG、人物 Skill、意图路由、混合检索、重排序、情景记忆和前端交互，实现可选择角色、可选择时间阶段、可切换知识模式的沉浸式对话体验。
...
- 角色对话：支持罗辑、章北海等人物对话入口，前端可以进入不同角色聊天页。
...
- 前端交互系统：提供登录页、角色选择页、罗辑聊天页和章北海聊天页，支持网页化体验。
...
3. Chat Service 创建对应角色的 Agent Shell，例如 LuoJiAgent 或 ZhangBeihaiAgent。
...
普通角色扮演只需要告诉模型“你是罗辑”，但这个项目需要解决“你是哪个阶段的罗辑”。罗辑在不同阶段的心理状态、责任感、语言风格和知识范围都不同，如果不做时间线控制，模型很容易混合早期、威慑纪元和晚年罗辑的表达。
...
- 为更多角色构建时序 Persona Skill，例如叶文洁、王淼、程心等。
